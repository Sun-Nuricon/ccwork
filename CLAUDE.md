...
새 이슈를 작업할 때는 아래 순서를 **표준 흐름**으로 따른다. Claude는 이 사이클을
**능동적으로 제어**한다 — 한 단계가 끝나면 결과를 보고하고 **다음 단계가 무엇인지
제안**하되, 사용자가 승인하기 전에는 다음 단계로 넘어가지 않는다.

| #   | 단계                 | 호출                                                             | 종류  |
| --- | -------------------- | ---------------------------------------------------------------- | ----- |
| 1   | 시그니처 + 시나리오  | `/test-scenarios N`                                              | skill |
| 2   | 실패 테스트 작성     | `/tdd-red N`                                                     | skill |
| 3   | 최소 구현(전체 통과) | `/tdd-green N`                                                   | skill |
| 4   | AC 충족 독립 검증    | `@ac-verifier N`                                                 | agent |
| 5   | 구조 개선(롤백 가능) | `/tdd-refactor N`                                                | skill |
| 6   | 타입·보안 점검       | `/security-review N`                                             | skill |
| 7   | 통합·머지            | commit → PR `--base feature/<spec>` → squash merge → 이슈 클로즈 | —     |

원칙:

- **각 단계는 인간 승인 게이트다. 자동으로 다음 단계로 넘어가지 않는다.** 단계 완료 후
  멈춰서 결과를 보고하고, 다음 단계를 한 줄로 제안한 뒤 사용자의 진행 지시를 기다린다.
- **4번 `@ac-verifier`는 생략하지 않는다.** "테스트 통과 ≠ AC 충족"이므로, GREEN이
  초록이어도 AC 의도가 실제로 충족됐는지 독립 검증한다. 갭이 나오면 시나리오 보강
  (1번으로 되돌아가 RED→GREEN)으로 닫고 재검증한다.
- **빨간 테스트 상태에서 REFACTOR(5)로 가지 않는다.** 전부 초록일 때만 구조 개선을
  시작하고, 변경으로 테스트가 깨지면 즉시 롤백한다.
- **이슈 의존성**: 선행 이슈가 있으면 그 이슈가 **머지된 feature 브랜치에서 분기**해
  작업한다(예: 이슈 1이 머지된 브랜치 위에서 이슈 2를 시작).
- 단계별 상세 규약은 각 skill/agent 정의에 있다. 이 표는 **순서와 게이트**를 강제하는
  단일 기준이다.
